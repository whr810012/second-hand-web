...
app.post("/Location/subMarkers", upload.array('files'), async (req, res) => {
  try {
    ...
    // 检查是否有文件上传
    const filesBase64 = req.files ? await Promise.all(req.files.map(async (file) => {
    ...
    const result = await executeQuery(sql, [
      markerid, lat, lng,name,icon,detail, filesJson
    ]);
    // 清空uploads文件
...
app.post("/Location/updateMarkers", upload.array('files'), async (req, res) => {
  try {
    ...
    const result = await executeQuery(sql,[
      lat, lng,name,icon,detail, filesJson2, markerid
    ])
    // 清空uploads文件
    fs.readdirSync("uploads").forEach(file => {
      fs.unlinkSync(`uploads/${file}`);
